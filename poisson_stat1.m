function U = poisson_stat1(ht,dt,wt,n)
  
% Simulation du probleme de Poisson
% Probleme mixte: CL de Dirichlet non-homogene + Neumann homogene 
% Terme source localise (de type chauffage)
% Prise en compte des CL Dirichlet (ajout d'une fenetre et d'une porte)
...
G = ((X>-0.9) & (X<0) & (Y>-0.7) & (Y<0.8)) | ...  % geometrie de la chambre &
   ((X>0) & (X<0.7) & (Y>-0.4) & (Y<0.8)) | ... 
   ((X>0.7) & (X<0.9) & (Y>0.4) & (Y<0.8));
  ...
  for j = 2:n-1
    ...
    if ind ~= 0 
      if G(i-1,j) == 0                % la fenêtre se situe sur le mur du haut
        if (X(i,j)>0.1 && X(i,j)< 0.6) % longueur de la fenêtre 
          window = [window G(i,j)];   % indices correspondant à la fenêtre
        else
      ...
      if G(i,j+1) == 0                % la porte se situe sur le mur de droite
        if (Y(i,j)>0.5 && Y(i,j)<0.7) % longueur de la porte
          door = [door ind];          % indices correspondant a la porte
        else
          A(ind,ind) = A(ind,ind)-1;  % en dehors de la porte, le mur est isloant
        end
      end
    end
  end
