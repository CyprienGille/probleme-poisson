function U = poisson_instat_imp_factorise(ht, dt, wt, n) 
% Simulation instationaire du probleme de Poisson
% Version avec schema d'euler implicite factorise, chambre 1
%
...
U_exact = poisson_stat1(ht, dt, wt, n); %solution exacte pour vérifier la convergence
...
[L, Um, P] = lu(eye(size(A))-nu*deltat*A); % decomposition LU
tic;
while nb_step < 300 %critère d'arrêt à étapes arbitraire
%while abs(sum((U-U_exact)(:))) > 1 %critère d'arrêt à précision arbitraire             
    nb_step++;
    uold = u;
    u = Um\(L\(P*(uold - deltat*b))); % implicite factorisé
    U = G;
    U(G>0) = full(u(G(G>0)));
    mesh(X,Y,U);
    axis('ij');
    title(sprintf("Simulation par schema d'euler implicite factorisé, t=%d", nb_step));
    pause(0.01);
endwhile
time = toc