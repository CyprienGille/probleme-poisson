function U = poisson_instat1_exp(ht, dt, wt, n) 
% Simulation instationaire du probleme de Poisson
% Version avec schema d'euler explicite, chambre 1
%
...
%while abs(sum((U-U_exact)(:))) > 1 %critère d'arrêt à précision arbitraire             
    nb_step++;
    uold = u;
    u = (eye(size(A))+nu*deltat*A)*uold - deltat*b; % explicite
    U = G;
    U(G>0) = full(u(G(G>0)));
    mesh(X,Y,U);
    axis('ij');
    title(sprintf("Simulation par schema d'euler explicite, t=%d", nb_step));
    pause(0.01);
endwhile
time = toc